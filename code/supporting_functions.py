...
def update_rover(Rover, data):
    ...
    if Rover.start_time == None:
        ...
        samples_xpos = np.int_(
            [convert_to_float(pos.strip()) for pos in data["samples_x"].split(";")]
        )
        samples_ypos = np.int_(
            [convert_to_float(pos.strip()) for pos in data["samples_y"].split(";")]
        )
...
def create_output_images(Rover):
    ...
    cv2.putText(
        map_add,
        "Time: " + str(np.round(Rover.total_time, 1)) + " s",
        (0, 10),
    ...
    cv2.putText(
        map_add, "Rocks", (0, 55), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1
    )
    cv2.putText(
        map_add,
    ...
    cv2.putText(
        map_add,
        "  Collected: " + str(Rover.samples_collected),
        (0, 85),
        cv2.FONT_HERSHEY_COMPLEX,
        0.4,
        (255, 255, 255),
